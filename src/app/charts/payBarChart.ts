...
import { JUDET, ROMANIA_CODE } from "./mapChart";
...
export class PayBarChart {
  ...
  private addAxes(svg: any) {
    var xAxis = d3.axisBottom(this.xScale).scale(this.xScale);

    var yAxis = d3.axisLeft(this.yScale).scale(this.yScale).ticks(8);
  ...
  private addBars(
    ...
    bars
      .on("mouseover", (d: any) => {
        ...
        this.tooltip.html(d["value"]).style("visibility", "visible");
      })
      .on("mousemove", (d) => {
        this.tooltip
  ...
  private formatData() {
    ...
    return [
      {
        text: this.labelIndemnizati,
        value: parseInt(selectedAreaData[indemnizati].replace(",", "").trim()),
      },
      {
        text: this.labelNeindemnizati,
        value: parseInt(
          selectedAreaData[neindemnizati].replace(",", "").trim()
        ),
      },
    ];
  }
  ...
  public updateData(newData: any, selectedArea: string) {
    ...
    let xAxis = d3.axisBottom(this.xScale).scale(this.xScale);

    let yAxis = d3.axisLeft(this.yScale).scale(this.yScale).ticks(8);
