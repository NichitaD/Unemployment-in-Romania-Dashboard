...
export class LineChart {
  ...
  private createScales() {
    let yExtent = [
      d3.min(this.data, (d) => {
        return (
          this.formatValue(d.data[42][NR_TOTAL_SOMERI]) -
          this.formatValue(d.data[42][NR_TOTAL_SOMERI]) / 50
        );
      }) || 0,
      d3.max(this.data, (d) => {
        return (
  ...
  private addLine() {
    let line = d3
      ...
      .y((d: any) =>
        this.yScale(this.formatValue(d.data[42][NR_TOTAL_SOMERI]))
      );
    ...
    this.plot
      ...
      .attr("transform", `translate(${this.leftPadding}, 0)`)
      .on("mouseover", (d: any) => {
        let selectedState = d.data.find(
          (set: any) => set[JUDET].trim() == areaCode
        );
        this.tooltip.transition().duration(500).style("opacity", 0.85);
        this.tooltip
          .html(
            "<strong>" +
              selectedState[NR_TOTAL_SOMERI].replace(",", "").trim() +
              "</strong>"
          )
  ...
  public updateData(newData: any, selectedArea: string) {
    ...
    let areaCode =
      this.selectedArea == "ROMANIA" ? ROMANIA_CODE : this.selectedArea;

    let yExtent = [
      d3.min(this.data, (d, i) => {
        let selectedState = d.data.find(
          (set: any) => set[JUDET].trim() == areaCode
        );
        return (
          this.formatValue(selectedState[NR_TOTAL_SOMERI]) -
          this.formatValue(selectedState[NR_TOTAL_SOMERI]) / 50
        );
      }) || 0,
      d3.max(this.data, (d) => {
        let selectedState = d.data.find(
    ...
    this.plot.selectAll(".y-axis").call(yAxis);
