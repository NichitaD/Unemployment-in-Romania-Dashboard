...
export class MapChart {
  ...
  private draw() {
    ...
    let offset: [number, number] = [
      this.width / 2,
      screensize < 1100 ? this.height - 140 : this.height - 200,
    ];
    let projection = d3
      .geoMercator()
      .center(center)
    ...
    this.svg
      ...
      .on("mousemove", function (d, i) {
        ...
        tooltip
          .style("left", xPosition + 9 + "px")
          .style("top", yPosition - 60 + "px");
      })
      .on("click", (d: any, i) => {
        let event = new Event("changedArea") as any;
        event.selectedArea =
          d.properties.name.toLowerCase() === this.selectedArea.toLowerCase()
            ? "Romania"
            : d.properties.name;
  ...
  private buildBottomText(data: any) {
    ...
    // Build bottom text
    $("#mapText").html(`<strong>${
      selectdeAreaData[NR_TOTAL_SOMERI]
    }</strong> of the citizens of <strong>${
      this.selectedArea
    }</strong> were unemployed in <strong>${this.selectedMonth}</strong>,
                            totaling up to <strong>${selectdeAreaData[
                              RATA
                            ].trim()}%</strong> of the population.`);
  }
  ...
  public updateData(
    ...
    this.svg.selectAll("path").attr("fill", (d: any) => {
      let value = d.properties.nrSomeri;
